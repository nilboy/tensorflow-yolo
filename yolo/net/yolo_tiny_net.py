...
class YoloTinyNet(Net):
  ...
  def body1(self, num, object_num, loss, predict, labels):
    ...
    for y in range(self.cell_size):
      for x in range(self.cell_size):
        base_boxes[y, x, :] = [self.image_size / self.cell_size * y, self.image_size / self.cell_size * x, 0, 0]
    ...
    #calculate truth C [CELL_SIZE, CELL_SIZE, BOXES_PER_CELL]
    C = iou_predict_truth
    ...
    return num + 1, object_num, loss + class_loss + object_loss + noobject_loss + coord_loss, predict, labels
  ...
  def loss(self, predicts, labels, objects_num):
    ...
    for i in range(self.batch_size):
      ...
      tuple_results = tf.while_loop(self.cond1, self.body1, [tf.constant(0), object_num, loss, predict, label])
      loss = tuple_results[2]
